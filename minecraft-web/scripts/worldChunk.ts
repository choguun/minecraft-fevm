...
export class WorldChunk extends THREE.Group {
  ...
  params: {
    seed: number; terrain: {
      scale: number; magnitude: number; offset: number; waterHeight: number;
    }; trees: {
      frequency: number; trunkHeight: { min: number; max: number; }; canopy: {/**
 * Initializes an empty world
 */ size: { min: number; max: number; }; density: number;
      };
    }; clouds: {
      density: number; scale: number;
    };
  };
  loaded: boolean;
  dataStore: DataStore;

  constructor(size: { width: number; height: number; }, params: {
      seed: number; terrain: { scale: number; magnitude: number; offset: number; waterHeight: number; }; trees: {
        frequency: number; trunkHeight: { min: number; max: number; }; canopy: { /**
 * Initializes an empty world
 */ size: { min: number; max: number; }; density: number;
        };
      }; clouds: { density: number; scale: number; };
    }, dataStore: DataStore) {
    super();
    this.size = size;
    this.params = params;
   ...
   */
  generateResources = (rng: object | undefined) => {
   ...
   * @param {RNG} rng Random number generator
   */
  generateTerrain = (rng: object | undefined) => {
    const simplex = new SimplexNoise(rng);
    for (let x = 0; x < this.size.width; x++) {
      for (let z = 0; z < this.size.width; z++) {
   ...
   * @param {RNG} rng 
   */
  generateClouds = (rng: object | undefined) => {
    const simplex = new SimplexNoise(rng);
    for (let x = 0; x < this.size.width; x++) {
      for (let z = 0; z < this.size.width; z++) {
        const value = simplex.noise(
  ...
  generateMeshes = () => {
    ...
    Object.values(blocks)
      .filter((blockType: any) => blockType.id !== blocks.empty.id)
      .forEach((blockType) => {
        const maxCount = this.size.width * this.size.width * this.size.height;
        const mesh: any = new THREE.InstancedMesh(geometry, blockType.material, maxCount);
  ...
  addBlockInstance = (x: number, y: number, z: number) => {
    ...
    if (block && block.id !== blocks.empty.id && block.instanceId === null) {
      ...
      const mesh: any = this.children.find((instanceMesh) => instanceMesh.name === block.id);
      const instanceId = mesh.count++;
      this.setBlockInstanceId(x, y, z, instanceId);
