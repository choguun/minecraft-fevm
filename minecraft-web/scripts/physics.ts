...
export class Physics {
   ...
   * @param {Player} player
   */
  resolveCollisions = (collisions: { block: any; contactPoint: { x: number; y: number; z: number; }; normal: THREE.Vector3; overlap: number; }[], player: Player) => {
    // Resolve the collisions in order of the smallest overlap to the largest
    collisions.sort((a, b) : any => {
   ...
   * @returns {boolean}
   */
  pointInPlayerBoundingCylinder = (p: { x: any; y: any; z: any; }, player: { position: { x: number; y: number; z: number; }; height: number; radius: number; }) => {
